...
public class Main2ProgramowanieDynamiczne {
    ...
    public static void main(String args[]) {
        ...
        // p6:
        Item p6_1 = new Item(12.0, 10.0, 5);
        ...
        for (int i = 0; i <= n; i++) {
            for (int j = 0; j <= m; j++) {
                ...
                // jesli waga i-1 produktu jest mniejsza od j
                    ...
                    if ((tab[i - 1][j] < ((int)produkty.get(i - 1).getPrice() + tab[i - 1][j - (int)produkty.get(i - 1).getWeight()]))) {
                        k[i][j] = 1;
                    }
                    else {
                        k[i][j] = 0;
